	...
	$.fn.mapSearch = function ( options ) {
			...
			*/
			settings = $.extend({
				...
				*/
				icon : '/templates/frontend/map_search/img/normal.png',	//Icon to be used on map
				
				/*
				*String : Path to highlighted marker icon to be used for each listing
				*/
				highlighted_icon : '/templates/frontend/map_search/img/highlight.png', 
			...
			*/
			function request_listings( new_params ){
				...
				if(document.getElementById('dont_reload_map').value==1){
				   document.getElementById('dont_reload_map').value=0;
				   return false;	
				}
				 setTimeout(function(){
					 var listLen = document.getElementById('ms-listings').innerHTML;
				  	 
					 if(listLen.length==0){
					 	$(el).removeClass( settings.loading_class );
						$(settings.listings_el).removeClass( settings.loading_class );
						document.getElementById('ms-listings').innerHTML="<br><center><b>No result found</b></center>";
					 }
				 },5000);
			...
			*/
			function addMarker( index , listing ) {
			  ...
			  var marker = new google.maps.Marker({
				...
				google.maps.event.addListener(marker, 'click', function() {
				  document.getElementById('dont_reload_map').value=1;
				});
